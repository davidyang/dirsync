...
$LOG = Logger.new('gconomy.log', 'monthly')

basedir="/chroots"

partners = [ "kashless" ]
sync_to_rb = [ "uploads" ]
sync_from_rb = [ "log", "processed" ]
sync_two_way = []

scp_to

partners.each do |p|
  homedir = "#{basedir}/#{p}/home/#{p}/"
  
  # for each sync to RB, rsync files to remote machine, if successful, delete local files
  
  # for each sync from RB, rsync files, then if successful, ssh delete them
  
  # for two-way syncs, just run rsync
